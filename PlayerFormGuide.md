...
②の5連撃を追加済みです。連撃の設定と追加した斬撃エフェクトについては [ComboAttackGuide.md](ComboAttackGuide.md) を参照してください。以下は①の設定の説明です。

③のビームも追加済みです。魔法形態の射程・時間・倍率とHierarchyの設定は [BeamAttackGuide.md](BeamAttackGuide.md) を参照してください。

## 今回できること

強化アイテムを取得すると、指定した形態の通常・攻撃・被弾アニメーション、上下移動速度、攻撃の命中音／空振り音へ切り替わります。被弾音は全形態で共通です。

FiveHitは5連撃、Beamは専用ビームに対応しています。Normal・Finaleは既存の単発攻撃です。自動発射・終盤イベントは④で実装します。通常アイテム・ハートの効果は従来どおり加点のみで、回復機能は追加していません。
...
- `Kind`：形態の識別名。FiveHitの場合は5連撃を行います。
...
通常・攻撃クリップは基本的に既存仕様に合わせた1枚絵です。FiveHitの攻撃クリップは②で攻撃絵と通常絵の交互表示に変更しました。被弾クリップには各形態の通常画像と点滅を設定しています。専用の連番アニメーションができたらクリップを置き換えられます。
